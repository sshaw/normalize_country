...
A small script is included that can convert country names contained in a DB table or a set of XML or CSV files
  ...
  usage: normalize_country [options] SOURCE
      ...
      -t, --to CONVERSION              Convert country names to this format 
      	       			       (see docs for valid formats)
...
For +csv+ and +xml+ the original file(s) will be overwritten with new file(s) containing the converted country names. 
The +db+ format will update the database column with the converted name.
